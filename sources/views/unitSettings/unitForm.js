...
import {roleStorage} from "models/unit_settings/roleStorage";

export default class unitForm extends JetView{
	config(){
			...
			elements:[
				...
				{cols:[
					{view:"button", value:"Сохранить", hotkey: "enter", click: () => {
						...
						if (form.isDirty()){
							if (form.validate()){
								let unit = form.getValues();
								unit.work_begin = unit.work_begin.getHours()+':'+unit.work_begin.getMinutes();
								unit.work_end = unit.work_end.getHours()+':'+unit.work_end.getMinutes();
								this.app.callEvent("save:unitForm",[unit]);
								//this.view.hide();
							} else	webix.message({ type:"error", text:"Заполните все поля"});
						}
					}},
					{view:"button", value:"Отмена", hotkey: "esc", click: () => {
						this.app.callEvent("cancel:unitForm");
						this.view.hide();
					}}
				]},
				{}
