	...
	config(){
		return {
			...
			template: (obj) => {
				...
				return `<div style="${cssCell}"><span style="${dev_type.font_style}">${dev_type.dev_type}</span> ${(obj.param)?obj.param+' '+dev_type.param_measure:''}<span class="webix_icon mdi mdi-close remove-icon" title="Remove"></span></div>`;
			},
			onClick:{
				"remove-icon":(e, id) => {
					developmentsStorage.waitSave( () => {
						developmentsStorage.remove(id);
					})
					.then((data) => {
						console.log("remove-icon", data);
			...
			on:{
				onAfterSelect: (id) => {
					developmentsStorage.setCursor(id);
				}
			},
			data:[]
		}
